...
export interface Options {
  ...
  /**
   * Defines if a array of names should be generated
   * @default false
   */
  generateConsts?: boolean
  /**
   * Name of the const to be used when generateConsts is set to true
   * @default 'icons'
   */
  constName?: string
  /**
   * File name of the generated consts file
   * @default 'icons'
   */
  constFileName?: string
  /**
   * Name of the output directory for generated consts file
   * @default 'src'
   */
  constOutputDir?: string
}
...
async function generateSvgSprite(options: Required<Options>): Promise<string> {
  ...
  if (generateConsts) {
    ...
    const constOutputDir = options.constOutputDir || dirname(icons)

    if (!existsSync(constOutputDir))
      mkdirSync(constOutputDir, { recursive: true })

    writeFileSync(
      normalizePath(`${resolve(root, constOutputDir)}/${constFileName}.ts`),
      fileComment + fileContent,
    )
  }
